window.addEventListener('DOMContentLoaded', () => {
  ...
  transports: ["websocket"]
    ...
    socket.on("playing", (e) => {
        ...
        const myTurn = (myValue === currentTurn);
        
        // FIX: Assign to global opponentName (not shadowed)
        opponentName = (foundObject.p1.p1name === userName) ? foundObject.p2.p2name : foundObject.p1.p1name;
        ...
        if (addTimeBtn && removeTimeBtn) {
            // Add Time logic — can only use during own turn
            addTimeBtn.onclick = () => {
                if (!myTurn) {
                    alert("❌ Not your turn!");
                    return;
                ...
                if (usedAddTime) {
                    alert("🕒 You've already used Add Time!");
                    return;
                }
                ...
                alert("✅ Added 10s to your turn!");
            };
    
            // FIXED: Remove Time logic — can only use when NOT your turn
            removeTimeBtn.onclick = () => {
                if (myTurn) {
                    alert("❌ You can only remove time when it's NOT your turn!");
                    return;
                }
    
                if (usedRemoveTime) {
                    alert("⚠️ You've already used Remove Time!");
                    return;
                }
    
                socket.emit("removeTime", { target: opponentName, userName });
                usedRemoveTime = true;
                removeTimeBtn.disabled = true;
                alert("⏱ Removed 10s from opponent!");
            };
        }
    ...
    socket.on("removeTime", ({ target }) => {
        if (userName === target && turnTime > 10) {
            turnTime -= 10;
            updateTurnTimerDisplay();
            alert("⚠️ Opponent used a power-up to remove 10s!");
        }
    });
